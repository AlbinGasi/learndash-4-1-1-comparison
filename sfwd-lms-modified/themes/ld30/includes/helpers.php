...
function tb_lms_enqueue_admin_script( $hook ) {
    $theme_template_url = LearnDash_Theme_Register::get_active_theme_base_url();
    wp_register_script( 'tb-lms-custom-js', $theme_template_url . '/assets/js/tb-lms-custom.js', array( 'jquery' ), LEARNDASH_SCRIPT_VERSION_TOKEN, true );
    wp_enqueue_script( 'tb-lms-custom-js' );

    wp_enqueue_style('tb-lms-custom-style', $theme_template_url . '/assets/css/tb-lms-custom.css', array(), LEARNDASH_SCRIPT_VERSION_TOKEN);
}
add_action( 'admin_enqueue_scripts', 'tb_lms_enqueue_admin_script' );
/**
