...
add_action(
    'wp_ajax_learndash_builder_view_page',
    function() {
        $all_id = $_POST['data'] ?? null;
        $result = [];
        if ( isset( $all_id ) ){
            foreach ( $all_id as $value ){
                $result[] = get_the_permalink( (int)$value );
            }
        }

        $tb_status = ['status' => 'success','msg' => $result];

        if ( empty( $result ) ){
            $tb_status = ['status' => 'error' ];
        }

        echo wp_json_encode( $tb_status );
