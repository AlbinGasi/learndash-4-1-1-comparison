	...
	class Learndash_Admin_Menus_Tabs {
		...
		public function learndash_admin_menu_early() {
			...
			if ( ! empty( $add_submenu ) ) {
				...
				add_menu_page(
					esc_html__( 'Teach Beacon LMS', 'learndash' ),
					esc_html__( 'Teach Beacon LMS', 'learndash' ),
					'read',
					'learndash-lms',
					null,
		...
		protected function admin_header_panel( $menu_tab_key = '' ) {
			...
			$header_data = array(
				...
				'i18n'           => array(
					...
					'unsaved_chages'                     => esc_html__( 'You have unsaved changes. If you proceed, they will be lost.', 'learndash' ),
			...
			if ( 'archive' === $logic_control ) {
                ...
                if ( $screen_post_type === "sfwd-quiz" ||  $screen_post_type === "sfwd-topic" ){
                    $course_id = $_GET['course_id'] ?? null;
                    if ( isset( $course_id ) ) {
                        $header_data['back_to_url']     = admin_url( 'post.php?post='. $course_id .'&action=edit&currentTab=learndash_course_builder' );
                        $header_data['back_to_title']   = esc_html__( 'Back to Course Outline', 'learndash' );
                    }
                }

                if ( $screen_post_type === "sfwd-question" ){
                    $_quiz_id = $_GET['quiz_id'] ?? null;
                    if ( isset( $_quiz_id ) ) {
                        $header_data['back_to_url']     = admin_url( 'post.php?post='. $_quiz_id .'&action=edit&currentTab=learndash_quiz_builder' );
                        $header_data['back_to_title']   = esc_html__( 'Back to quiz', 'learndash' );
                    }
                }
